...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/politics_feeds_list_feeds \
  -H 'Content-Type: application/json' \
  -d '{"category":"politics"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/politics_feeds_list_feeds`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "politics-feeds": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-politics-feeds"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-politics-feeds
```
